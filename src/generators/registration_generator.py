...
class RegistrationGenerator(BaseGenerator):
    ...
    def generate(self) -> list[Registration]:
        ...
        for student in self.students:
            ...
            profile = self.profile_map.get(student.student_id)

            if profile is None:
                continue
            
            config = PROFILE_CONFIG[profile.profile_name]
            ...
            for offering in semester_offerings:
                ...
                mentor_id=offering.faculty_id,

                attendance_percentage=attendance,

                assignment_completion=assignment,

                lms_activity_score=RandomUtils.integer(
                    config["lms"][0],
                    config["lms"][1],
                ),
                ...
                registration_status=(
                    "CANCELLED"
                    if random() < config["cancel"]
                    else "REGISTERED"
                ),

                is_repeat=(
                    random() < config["repeat"]
                ),
            )
